import type { APIApplicationCommandGuildInteraction, APIChatInputApplicationCommandGuildInteraction, APIMessage } from "@discordjs/core";
import { ApplicationCommandOptionType, ApplicationCommandType, ChannelType, InteractionContextType, MessageFlags, PermissionFlagsBits } from "@discordjs/core";
import ApplicationCommand from "../../../../lib/classes/ApplicationCommand.js";
import type Language from "../../../../lib/classes/Language.js";
import type { APIInteractionWithArguments } from "../../../../typings/index.js";
import { PoddyClient } from "../../../client.js";
import PoddyFunctions from "../../../utilities/functions.js";
import { processRunPodQuery, ingestDocument } from "../../../utilities/runpod.js";
...
export default class Save extends ApplicationCommand {
	...
	public override async run({
		...
		const title = interaction.arguments.strings?.[
			this.client.languageHandler.defaultLanguage!.get(
				"SAVE_COMMAND_TITLE_OPTION_NAME",
			)
		]?.value
		
		const tags = interaction.arguments.strings?.[
			this.client.languageHandler.defaultLanguage!.get(
				"SAVE_COMMAND_TAGS_OPTION_NAME",
			)
		]?.value

		const prompt = interaction.arguments.strings?.[
			this.client.languageHandler.defaultLanguage!.get(
				"SAVE_COMMAND_PROMPT_OPTION_NAME",
			)
		]?.value

		const threadMessages = await (this.client.functions as PoddyFunctions).fetchAllThreadMessages(interaction.channel.id, {
			excludeBots: true,
		});

		if(threadMessages.length === 0) {
			return await this.client.api.interactions.reply(interaction.id, interaction.token, {
	...
	private formatMessages(messages: APIMessage[], interaction: APIInteractionWithArguments<APIApplicationCommandGuildInteraction>) {
		...
		try {
			...
			const truncatedConversation =
				conversation.length > maxLength
					? conversation.substring(0, maxLength) + '...[truncated]'
					: conversation;
	...
	private async sendAssistantChannelConfirmation(document: any) {
		...
		try {
			...
			if (document.summary_prompt && document.summary_prompt !== 'Default summary (main topic, solutions, technical details, outcomes)') {
