...
import { ChannelType, AuditLogEvent, GatewayDispatchEvents } from "@discordjs/core";
...
export default class GuildAuditLogEntryCreate extends EventHandler {
  ...
  public override async run({ data: auditLogEntry }: WithIntrinsicProps<GatewayGuildAuditLogEntryCreateDispatchData>) {
    ...
    if (auditLogEntry.action_type === AuditLogEvent.ChannelCreate) {
            ...
            embeds: [
              {
                ...
                description: `**User:** ${
                  auditLogEntry.user_id ? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\`` : "Unknown User"
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.ChannelDelete) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            if (change.key === "name") return { name: `**Channel Name:** ${change.old_value}`, order: 1 };
            else if (change.key === "type")
              return { name: `**Channel Type:** ${this.channelTypesMap[change.old_value as ChannelType]}`, order: 2 };
            else if (change.key === "nsfw") return change.old_value ? { name: `**NSFW:** Yes`, order: 3 } : null;
            else if (change.key === "rate_limit_per_user")
              return change.old_value
                ? { name: `**Slowmode:** ${this.client.functions.format(change.old_value * 1_000)}`, order: 4 }
                : null;
            else if (change.key === "topic")
              return change.old_value ? { name: `**Topic:** ${change.old_value}`, order: 5 } : null;
            else if (change.key === "user_limit")
              return change.old_value ? { name: `**User Limit:** ${change.old_value}`, order: 6 } : null;
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.ChannelUpdate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            if (change.key === "name")
              return { name: `**Channel Name:** ${change.old_value} -> ${change.new_value}`, order: 1 };
            else if (change.key === "type")
              return {
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.RoleCreate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
          event: "ROLE_CREATED",
          guildId: auditLogEntry.guild_id!,
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.RoleDelete) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
          event: "ROLE_DELETED",
          guildId: auditLogEntry.guild_id!,
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.RoleUpdate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
          event: "ROLE_UPDATED",
          guildId: auditLogEntry.guild_id!,
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.MemberRoleUpdate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            ...
            else if (change.key === "$remove" && change.new_value)
              return {
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.MemberUpdate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.ThreadCreate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            ...
            else if (change.key === "auto_archive_duration" && change.new_value)
              return {
            ...
            else if (change.key === "rate_limit_per_user" && change.new_value)
              return {
                name: `**Thread Slowmode:** ${this.client.functions.format(change.new_value * 1_000)}`,
                order: 4,
              };
            // @ts-expect-error
            else if (change.key === "invitable" && change.new_value)
              return {
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.ThreadUpdate) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            ...
            // @ts-expect-error
            else if (change.key === "invitable")
              return {
                // @ts-expect-error
                name: `**Thread Invitable:** ${change.old_value ? "Yes" : "No"} -> ${change.new_value ? "Yes" : "No"}`,
                order: 7,
              };
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.ThreadDelete) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
        ...
        auditLogEntry.changes
          ?.map((change) => {
            ...
            // @ts-expect-error
              ...
              return {
                // @ts-expect-error
                name: `**Thread Invitable:** ${change.old_value ? "Yes" : "No"}`,
                order: 7,
              };
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.InviteCreate) {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            ...
            else if (change.key === "max_uses" && change.new_value)
              return {
                name: `**Max Uses:** ${change.new_value}`,
                order: 3,
              };
            else if (change.key === "max_age" && change.new_value)
              return {
            ...
            else if (change.key === "temporary" && change.new_value)
              return {
    ...
    } else if (auditLogEntry.action_type === AuditLogEvent.InviteDelete) {
      const loggingChannels = await this.client.prisma.logChannel.findMany({
        where: {
      ...
      const changes = (
        auditLogEntry.changes
          ?.map((change) => {
            ...
            else if (change.key === "max_uses" && change.old_value)
              return {
                name: `**Max Uses:** ${change.old_value}`,
                order: 3,
              };
            else if (change.key === "max_age" && change.old_value)
              return {
                name: `**Expires:** ${this.client.functions.generateTimestamp({
            ...
            else if (change.key === "temporary" && change.old_value)
              return {
