...
import type { APIApplicationCommandAutocompleteInteraction, APIGuildForumChannel } from "discord-api-types/v10";
...
export default class AddTagsForForumChannel extends AutoComplete {
	...
	/**
	 * Create our remove tags for forum channel autocomplete.
	 *
	 * @param client - Our extended client.
	 */
	public constructor(client: ExtendedClient) {
		super(["config-auto_tag_on_forum_channel-remove-tag"], client);
	}
	...
	public override async run({
		...
		const autoTagsOnForumChannel = await this.client.prisma.autoTagOnForumChannel.findMany({ where: { channelId } });
		const tagIds = autoTagsOnForumChannel.map((tag) => tag.tagId);
		...
		const tags = (await this.client.api.channels.get(channelId)) as APIGuildForumChannel;
		this.cache.set(
			channelId,
