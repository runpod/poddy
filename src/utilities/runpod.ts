import { BOT_ACTION_PHRASES, QUERY_PROMPTS } from "./qa/messages.js";
...
const RUNPOD_INGESTION_ENDPOINT =
	process.env.RUNPOD_INGESTION_ENDPOINT || "https://api.runpod.ai/v2/t2dx8r4x8b2m5l/run";
 ...
 */
async function handlePassthroughResponse(
	question: string,
	threadContext: string | null,
	images: string[] | null = null,
	useComplex = false,
	...
	try {
		...
		const formattedConversation = document.full_conversation
			.map((msg: any) => {
				...
				const authorLabel = msg.author.bot ? `${msg.author.username} (Bot)` : msg.author.username;
